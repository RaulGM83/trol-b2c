-- 100 — Regla de negocio (Raúl, 1-sep-2026): cuando hay `fin_conservacion`,
-- ESA FECHA manda. `conserva_derechos` es una foto del momento en que se hizo
-- el cálculo y se queda vieja; la fecha sigue siendo cierta con el paso del
-- tiempo. Antes el motor miraba solo la bandera, así que a 11 personas con la
-- fecha ya pasada les seguía diciendo "derechos vigentes" y les abría
-- pension_hoy. Sin fecha, la bandera sigue siendo lo único que hay.
...
declare e record; codigos text[] := '{}'; u_mej jsonb; mej_min numeric; mej_max numeric; ident text;
        um text; m40_vencida boolean := false; ya_pens boolean := false; derechos_ok boolean;
begin
  ...
  derechos_ok := case when e.fin_conservacion is not null
                      then e.fin_conservacion >= current_date
  ...
  -- 100: vencidos por fecha, o por vencer dentro de 180 días.
  if not ya_pens and e.ley = 'Ley73' and (not derechos_ok or (e.fin_conservacion is not null and e.fin_conservacion < current_date + 180)) then
    codigos := codigos || trol3._up_op(p_id, e.cabecera_id, 'reactivar_derechos', coalesce(e.pension_base,0)*12, jsonb_build_object('pension_base', e.pension_base, 'fin_conservacion', e.fin_conservacion),
            case when not derechos_ok then 'Derechos no vigentes: reactivar para pensión Ley 73' else 'Derechos por vencer' end, e.fin_conservacion);
  end if;
