-- 128b — `alta_aliado` no existía; el valor bueno era `alta_manual`.
--
-- 122 ya había definido los tres orígenes posibles de un referido —link, qr y
-- alta_manual— y 128 inventó un cuarto que el check rechaza. Se corrige
-- usando el que ya estaba, no ensanchando el check: un alta tecleada es un
-- alta tecleada, la haya escrito el aliado o nosotros. Quién la escribió ya
-- queda en el evento `alta_por_aliado`.
...
begin
  ...
  v_ref := trol3.registrar_referido(v_aliado, pid, 'alta_manual');
