...
2. 🟡 **Pasarela Mercado Pago (código completo 22 jun)** — **SPEI nativo como método principal** (§13): `app/api/pago/spei` (pago `payment_method_id:'clabe'` → CLABE en nuestra pantalla); tarjeta vía `app/api/pago/crear` (Checkout Pro). `app/api/pago/webhook` consulta el pago y dispara el fulfillment. Llaves puestas en `.env.local`. **Notas de prueba:** MP rechaza `notification_url` en localhost (lo omitimos si el sitio no es https) y **SPEI no se puede probar en sandbox** ("Unauthorized use of live credentials"); el código está validado hasta MP → la **prueba live de SPEI se difiere al deploy** (`app.trol.mx` + producción + webhook público; producto a $1 para la prueba). TODO: validar firma `x-signature`.
...
6. 🟡 **Dominio `app.trol.mx` + deploy (Vercel)** — repo preparado (raíz = `b2c experiencia` con `pension-core/` + `trol-b2c/`, Vercel Root Dir = `trol-b2c`); `.gitignore` protege secretos; **guía completa en `DEPLOY.md`** (GitHub → Vercel → env prod → DNS CNAME → MP producción + webhook → prueba SPEI a $1). Ejecuta el deploy el usuario (sus cuentas Vercel/DNS/GitHub).
