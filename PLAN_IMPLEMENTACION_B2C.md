...
4b. ✅ **Fulfillment + refresh Jordan (23 jun)** — RPC `procesar_pago_orden(orden, ref)` (service-role only, idempotente con lock `FOR UPDATE`): marca orden cumplida, **cashback 10%** (solo pago; caduca 6m) y **avanza `etapa_actual`** (verificado en el pago real). Refresh: si la semilla tiene **+1 mes**, el webhook **marca `clientes.sisec_refresh_solicitado_at = now()`** (activa el `mass_refresh` existente del cliente) y dispara el workflow **"Refrescar SISEC"** (`N8N_FULFILLMENT_URL`) con `{curp, cliente_id, origen:'b2c_pago'}` → Jordan trae datos al día → Waterfall PDF Jordan → Calculos recalcula la semilla **en silencio** (no notifica al usuario; las campañas con aviso serán otro diseño/etapa). La columna no se limpia (silencio por defecto, aceptado). Reutiliza los workflows B2B existentes.
...
4. ✅ **Ruteo Llave 3 — motor + página de asesores (23 jun)** — vista `vista_ruteo` (security_invoker, respeta RLS) clasifica cada cliente con semilla en `producto_estrella` + `motivo` + `prioridad` (puntaje_num), matriz §7. **Distribución (7,194):** Gestoría IMSS 3,622, Asesoría 2,406, Gestoría Infonavit 900, **Modalidad 40 201 (181 alta)**, PPR 65. **Superficie:** página `/ruteo` en `trol-portal` (chips por producto con conteos, tabla ordenada por prioridad, link a la ficha; solo asesores) + ítem de nav. Pendiente: que el usuario haga push del repo `trol-portal` para desplegarla. Opcional futuro: enriquecer la mejor jugada B2C con el ruteo.
5. 🔜 **Llave 4 — comisiones [DIFERIDO 23 jun]** — activar handoff/comisiones desde el ruteo (`delegations` interno, u `oportunidades`/HubSpot deal con comisión). Diferido por decisión del usuario: hoy los asesores ya tienen la vista `/ruteo` para trabajar la base. Estructuras listas: `oportunidades` (espejo HubSpot, `comision_pct/monto`), `delegations` (handoff interno), `comisiones_cartera` (recurrentes).
...
### Tarjeta in-page con MP Bricks [DECIDIDO 23 jun, pendiente build]
Pasar el pago con tarjeta de redirect (Checkout Pro) a **in-page con Card Payment Brick** (no romper la costura/confianza). Frontend: SDK MP + Brick (iframe seguro, PCI lo maneja MP) en la pestaña "Tarjeta". Backend: ruta `/api/pago/tarjeta` que recibe el token del Brick → crea el pago `/v1/payments` con la orden → aprueba al instante → misma pantalla de éxito (desbloqueo + cashback + refresh). Opcional: meses sin intereses para $500/$800. El webhook ya cubre tarjeta. Razonamiento del usuario: margen alto (comisión no importa) y costo hundido (contracargo razonable). SPEI sigue como método principal.
