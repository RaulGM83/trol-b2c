// ==========================================================================
// CALCULADORA DE PENSIÓN PRO - VERSIÓN COMPLETA v5.5
// NUEVO v5.5 — LÍNEAS DE CAPTURA DÍA A DÍA (24-ago-2026)
//   Espejo de pension-core mod40-lineas.ts, goldens del Excel validado al
//   centavo (claude/21-lineas-captura-dia-a-dia-spec.md):
//   1) calculateRetroCostBreakdown: prorrateo diario en los dos extremos (el
//      mes del trámite cobra días transcurridos; el de la baja, días después
//      de la baja), días reales por mes, actualización INPC, recargos 1.47%
//      con precisión diaria y TOPE 60 MESES (art. 219) conservando recientes.
//   2) INPC_HISTORY = serie de trol3.inpc_mensual (observado INEGI a 2026-03,
//      proyección ~0.327%/mes después). Actualizar junto con esa tabla.
//   3) getMod40Rate con los factores del Excel validado (10.08 vs 10.075).
//   4) esRegistroMod40: doble señal (RP 9999940$ o CONTINUACION VOLUNTARIA)
//      en lugar de endsWith("40"), que daba falsos positivos.
//   Entradas y salidas SIN CAMBIOS (mismas llaves).
// ==========================================================================
// (v5.4 abajo)
...
function getMod40Rate(year) {
    // v5.5: factores del Excel validado contra líneas reales (= pension-core mod40-lineas)
    if (year <= 2022) return 10.08;
    if (year === 2023) return 11.17;
    if (year === 2024) return 12.26;
    if (year === 2025) return 13.35;
    if (year === 2026) return 14.44;
    if (year === 2027) return 15.53;
    if (year === 2028) return 16.62;
    if (year === 2029) return 17.71;
    return 18.80;
}
// v5.5: detección Mod 40 de doble señal (espejo de trol3.derivar_ultima_modalidad
// y pension-core ventanaMod40). endsWith("40") daba falsos positivos con RPs
// normales que terminan en 40 por azar.
function esRegistroMod40(job) {
    const reg = String(job.employer_registration || "");
    if (/9999940$/.test(reg)) return true;
    const emp = String(job.employer || "").toUpperCase();
    return emp.indexOf("CONTINUACION VOLUNTARIA") !== -1 || emp.indexOf("CONTINUACI\u00d3N VOLUNTARIA") !== -1;
}
...
class PensionCalculator {
    ...
    getLastMod40EndDate() {
        ...
        for (const job of hist) {
            if (!esRegistroMod40(job)) continue;
    ...
    calculateRetroCostBreakdown(startDate, endDate, baseSalary) {
        // v5.5: día a día, espejo de pension-core mod40-lineas (goldens del Excel
        // validado al centavo). Prorrateo diario en los dos extremos, días reales
        // por mes, actualización INPC, recargos 1.47% con días/30.4375 y tope de
        // 60 meses (art. 219) conservando los meses más recientes.
        const MESES_MAX_ART219 = 60;
        const c10 = new Date(startDate.getFullYear(), startDate.getMonth(), 1);
        const c11 = new Date(endDate.getFullYear(), endDate.getMonth(), 1);
        const nPeriodo = Math.max(0, (c11.getFullYear() - c10.getFullYear()) * 12 + (c11.getMonth() - c10.getMonth()) + 1);
        const nCobra = Math.min(nPeriodo, MESES_MAX_ART219);
        const diasEnMes = (d) => new Date(d.getFullYear(), d.getMonth() + 1, 0).getDate();
        const inpc_pago = getINPC(c11.getFullYear(), c11.getMonth() + 1);
        let total_base = 0, total_act = 0, total_rec = 0, months_paid = 0;
        for (let i = 1; i <= nCobra; i++) {
            const g = new Date(c11.getFullYear(), c11.getMonth() - (i - 1), 1);
            const dm = diasEnMes(g);
            const salarioMes = baseSalary * dm;
            // El mes de la baja (i === nPeriodo) solo se prorratea si sobrevive
            // al tope: cuando el corte muerde, el último mes cobrado va completo.
            let prorr = 1;
            if (i === 1) {
                prorr = endDate.getDate() / dm;
            } else if (i === nPeriodo) {
                const dmBaja = diasEnMes(startDate);
                prorr = (dmBaja - startDate.getDate()) / dmBaja;
            }
            const cuota_mensual = (getMod40Rate(g.getFullYear()) / 100) * salarioMes * prorr;
            let factor = inpc_pago / getINPC(g.getFullYear(), g.getMonth() + 1);
            if (factor < 1) factor = 1; // CFF 17-A: la actualización no reduce
            const actualizacion = (factor - 1) * cuota_mensual;
            const dias_retraso = Math.round((c11 - g) / 86400000);
            const recargos = (cuota_mensual + actualizacion) * 0.0147 * Math.max(0, dias_retraso / 30.4375);
    ...
    execute() {
        ...
        for (const job of (this.user_data.employment_history || [])) {
            ...
            if (!maxEndDateHist || ed > maxEndDateHist) {
                maxEndDateHist = ed;
                isAbsoluteLastJobMod40 = esRegistroMod40(job);
            }
        }
        ...
        return {
            metadata: {
                ...
                server_version: "2.5.0-lineas-dia-v55",
