...
import { marcarMiLinkCompartido } from '@/app/trabajo/actions';
...
function Copiable({
  ...
  label: string;
  url: string;
  /** Sólo el link directo avisa: es el que significa algo en la bitácora. */
  alCopiar?: () => void;
}) {
...
export function CompartirLinks({
  directo = null,
  expediente,
  referido,
  personaId,
  ...
  return (
    <section className="rounded-2xl border border-line bg-white p-5">
      ...
      {directo ? (
        <Copiable
          label="Su expediente · entra directo (7 días)"
          url={directo}
          alCopiar={personaId ? () => void marcarMiLinkCompartido(personaId) : undefined}
        />
      ) : null}
