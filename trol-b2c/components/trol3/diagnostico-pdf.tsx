...
const Sec = ({ titulo, children }: { titulo: string; children: any }) => (
  <View style={s.sec}>
    {/* 24pt: lo justo para que el título no se quede solo al pie, sin empujar
        a la hoja siguiente una sección que sí cabía —era lo que mandaba "Tu
        situación hoy" lejos de la tabla de datos. */}
...
export type DiagnosticoPdfInput = {
  ...
  capitulos: Capitulo[];
  /** Los bloques que el asesor apagó para este documento. */
  seccionesOff?: string[] | null;
};
...
export function diagnosticoDoc(d: DiagnosticoPdfInput) {
  ...
  const narr = (k: SeccionNarrativa) =>
    (d.narrativa?.[k] ?? '').trim() ? (
      <Sec titulo={TITULO_SECCION[k]}>
        <Parrafos texto={d.narrativa[k]} />
      </Sec>
    ) : null;

  /** Lo que el asesor dejó fuera de este PDF. */
  const on = (id: BloquePdf) => bloqueEncendido(d.seccionesOff, id);

  return (
    <Document title={`Diagnóstico Avanzado · ${d.cliente}`} author="El Trol financiero">
      {/* -------- El documento, en un solo hilo --------
          Antes eran tres <Page> fijas y el contenido chocaba con el corte: un
          título se quedaba solo al pie y su texto arrancaba hasta la hoja
          siguiente. Aquí todo fluye y sólo el anexo empieza en hoja nueva.
      ...
      <Page size="LETTER" style={s.page}>
        ...
        <View style={s.body}>
          ...
          {on('historia_laboral') ? narr('historia_laboral') : null}

          {on('historia_laboral_tabla') ? (
            <>
          ...
          {on('oportunidades_gestorias') ? narr('oportunidades_gestorias') : null}
          ...
          {on('oportunidades_infonavit') ? narr('oportunidades_infonavit') : null}
          ...
          {on('oportunidades_ahorro') ? narr('oportunidades_ahorro') : null}
      ...
      {on('educacion') && d.capitulos.length ? (
        <Page size="LETTER" style={s.page}>
