...
import { OportunidadEtapa, EtapaChip, type Motivo, type ProveedorOp, type ProductoGestoria } from '@/components/trol3/OportunidadEtapa';
...
export default async function Expediente({ params, searchParams }: { params: { id: string }; searchParams: { tab?: string } }) {
  ...
  const { data: productosGestoria } = await db.from('catalogo_productos_gestoria').select('codigo,nombre,honorario_default,costo_default').eq('activo', true).order('orden');
  const productos = ((productosGestoria ?? []) as Any[]).map((x) => ({ codigo: x.codigo, nombre: x.nombre, honorario_default: Number(x.honorario_default), costo_default: Number(x.costo_default) })) as ProductoGestoria[];
  const productoSugerido = (codigo: string) => { const c = catMap.get(codigo) as Any; return c?.producto && productos.some((x) => x.codigo === c.producto) ? (c.producto as string) : null; };
          ...
          <ul className="space-y-3">
            {opsAbiertas.map((o: Any) => (
              <li key={o.id} className="rounded-xl border border-line p-3">
                ...
                <OportunidadEtapa op={{ id: o.id, codigo: o.codigo, estado: o.estado, especialista_id: o.especialista_id, motivo_perdida: o.motivo_perdida, proveedor: o.proveedor, contactar_despues: o.contactar_despues, honorario_trol: o.honorario_trol, comision_pct_aliado: o.comision_pct_aliado, producto: o.producto, costo_gestoria: o.costo_gestoria, producto_sugerido: productoSugerido(o.codigo) }} productos={productos} conAliado={conAliado} personaId={e.persona_id} motivos={(motivosPerdida ?? []) as Motivo[]} proveedores={(provsOp ?? []) as ProveedorOp[]} miembros={(miembros ?? []).map((x: Any) => ({ id: x.id, nombre: x.nombre ?? x.email }))} />
